...
class DetectionLayer(KE.base_layer.Layer):
    ...
    def call(self, inputs):
        ...
        return tf.numpy_function(wrapper, inputs, tf.float32)
...
class MaskRCNN():
    ...
    def load_weights(self, filepath, by_name=False, exclude=None):
        ...
        """
        import h5py
        from keras.engine import saving # from keras.engine import topology
    ...
    def unmold_detections(self, detections, mrcnn_mask, image_shape, window):
        ...
        h_scale = np.float64(image_shape[0]) / np.float64(window[2] - window[0]) #Modified by Berta
        w_scale = np.float64(image_shape[1]) / np.float64(window[3] - window[1]) #Modified by Berta
